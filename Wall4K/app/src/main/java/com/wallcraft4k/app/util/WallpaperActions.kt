...
object WallpaperActions {
    ...
    suspend fun setParallaxWallpaper(context: Context, model: String): Boolean {
        // Screen-sized decode keeps the parallax file small and the service fast.
        val bitmap = loadBitmap(context, model, fitToScreen = true) ?: return false
    ...
    /**
     * Saves [model] into the public gallery under Pictures/Wall4K.
     *
    ...
    suspend fun saveToGallery(
        ...
        val fileName = "Wall4K_${safeName}_${System.currentTimeMillis()}.$ext"
    ...
    private fun openSourceStream(context: Context, model: String): InputStream {
        return if (model.startsWith("http://") || model.startsWith("https://")) {
            ...
            connection.setRequestProperty("User-Agent", "Wall4K/1.0")
